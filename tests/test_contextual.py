...
async def parse_doc(file_path: str, output_path: str):
    ...
    if hasattr(res, 'markdown_document'):
        ...
        if hasattr(res, 'document_metadata') and hasattr(res.document_metadata, 'hierarchy'):
            hierarchy_content = getattr(res.document_metadata.hierarchy, 'table_of_contents', '')
    ...
    if hierarchy_content:
        with open(output_path.replace('.json', '_hierarchy.md'), 'w', encoding='utf-8') as f_hierarchy:
            f_hierarchy.write(hierarchy_content)
        print(f"Saved table of contents to: {output_path.replace('.json', '_hierarchy.md')}")
...
if __name__ == "__main__":
    ...
    asyncio.run(parse_doc("tests/test_files/PGhandbook2025.pdf", "tests/output/PGhandbook2025.json"))
