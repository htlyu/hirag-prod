    ...
    def validate_config_based_on_service_type(self) -> "EmbeddingConfig":
        if self.service_type == "local":
            if not self.model_name:
                raise ValueError(
                    f"{EmbeddingConfig.Config.alias_generator("model_name")} is required when service_type is local"
                )
            if not self.model_path:
                raise ValueError(
                    f"{EmbeddingConfig.Config.alias_generator("model_path")} is required when service_type is local"
                )
